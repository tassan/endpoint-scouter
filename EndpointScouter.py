...
class EndpointScouter:
    def __init__(self, timeout=5, max_workers=10, rate_limit_test_count=10, dbz_mode=False):
        self.timeout = timeout  # Timeout for requests in seconds
        self.max_workers = max_workers  # Maximum number of threads for parallel requests
        self.rate_limit_test_count = rate_limit_test_count  # Number of requests to test rate limiting
    ...
    def _write_csv_report(self, csv_file):
        ...
        try:
            with open(csv_file, 'w', newline='') as f:
                ...
                # Header
                writer.writerow([
                    "URL", "Method", "Accessible", "Status Code", 
                    "Expected Status", "CORS Configured", "Rate Limit", 
                    "Security Headers", "Response Time (s)", "Errors"
                ])
    ...
    def _calculate_security_score(self):
        ...
        for r in self.results:
            ...
            if r.get("cors_headers"):
                origin_header = r.get("cors_headers", {}).get("Access-Control-Allow-Origin")
...
def main():
    ...
    parser.add_argument(
        "endpoints_file", 
        help="CSV or JSON file containing endpoints to check"
    )
    parser.add_argument(
        "-o", "--output", 
